...
app.post("/webhook", (req, res) => {
  ...
  const { taskId, filePath, agentId, destination, createdTime, currentState } = details.data;
  ...
  switch (type) {
    case "capture:available":
      ...
      console.log(`Downloading Recording for TaskId: ${taskId} from path: ${filePath} created On: ${new Date(createdTime)}`);
      console.log(org_id);
      // send email notification
      msg = emailNotify(`Recording Available`, `Recording ${taskId} create on ${createdTime}, location: ${filePath}`, `<strong>Recording: <br> ${taskId} <br> create on ${createdTime}, <br> location: ${filePath}</strong>`);
      break;
    case "agent:login":
      console.log(`Agent: ${agentId} using DN: ${destination} has login at: ${new Date(createdTime)}.`);
      // send email notification
      msg = emailNotify(`Logging in...`, `Agent: ${agentId} using DN: ${destination} has login at: ${new Date(createdTime)}.`, `<strong>Agent: <br> ${agentId} <br> DN: <br> ${destination} <br> Login at: <br> ${new Date(createdTime)}.</strong>`);
      break;
    case "agent:logout":
      console.log(`Agent: ${agentId} has ${currentState} at: ${new Date(createdTime)}`);
      // send email notification
      msg = emailNotify(`Logging out...`, `Agent: ${agentId} has ${currentState} at: ${new Date(createdTime)}`, `<strong>Agent: <br> ${agentId} <br> ${currentState} at: <br> ${new Date(createdTime)}</strong>`);
      break;
    default:
...
function processEvent(event) {
  ...
  switch (event.type) {
    ...
    default:
      console.warn(`Unexpected event type '${event.type}', ignoring. Maybe the Subscription is misconfigured?`);
      break;
  }
}
